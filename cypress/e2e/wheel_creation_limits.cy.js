...
import { TEST_USER, setupFreemiumUser, setupPremiumUser } from '../support/test-helpers';
...
describe('Wheel Creation and Limits', () => {
  ...
  describe('for Subscribing user', () => {
    beforeEach(() => {
      const premiumFixtures = setupPremiumUser(fixtures);
      
      // Set up common intercepts
      cy.stubAuthEndpoints(premiumFixtures);
      cy.stubUserData(premiumFixtures);
      cy.stubOrganizationData();
      
      // Intercept team invitations
      cy.intercept('GET', '**/rest/v1/team_invitations*', { statusCode: 200, body: [] }).as('teamInvitations');
      
      // Intercept activity groups
      cy.intercept('GET', '**/rest/v1/activity_groups*', { statusCode: 200, body: [] }).as('activityGroups');
    });

    it('allows unlimited wheel creation for premium users', () => {
      // Set up with 5 existing wheels (well beyond free limit)
      const fiveWheels = [
        fixtures.userWheels[0],
        fixtures.userWheels[1],
        { ...fixtures.userWheels[0], id: 'wheel-3', title: 'Wheel 3' },
        { ...fixtures.userWheels[0], id: 'wheel-4', title: 'Wheel 4' },
        { ...fixtures.userWheels[0], id: 'wheel-5', title: 'Wheel 5' },
      ];
      ...
      // Verify no limit message appears
      cy.contains('Nått gränsen för hjul').should('not.exist');
      ...
      cy.wait('@createWheel').then((interception) => {
        ...
        const wheel = Array.isArray(interception.response.body) 
          ? interception.response.body[0] 
          : interception.response.body;
        expect(wheel).to.have.property('title', 'Premium User Wheel 6');
      });
    ...
    it('does not show upgrade prompt for premium users', () => {
      // Set up with 10 wheels (way beyond free limit)
      const tenWheels = Array.from({ length: 10 }, (_, i) => ({
        ...fixtures.userWheels[0],
        id: `wheel-${i + 1}`,
        title: `Wheel ${i + 1}`
      }));
      ...
      // Verify create wheel card is still enabled (no limit message)
      cy.get('[data-cy="create-wheel-card"]').should('be.visible');
      cy.contains('Nått gränsen för hjul').should('not.exist');
