/**
 * Fetch First Row (Headers) from Google Sheet
 * Returns the first row of a specified sheet for column mapping
 */
...
serve(async (req) => {
  ...
  try {
    ...
    // Fetch first row (headers) from Google Sheets API
    // Range: A1:ZZ1 to get up to 702 columns
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to fetch sheet headers: ${response.status} ${response.statusText}`)
    }
