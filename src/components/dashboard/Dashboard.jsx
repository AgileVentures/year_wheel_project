      ...
      {isOpen && (
        <>
          ...
          <div className="absolute right-0 mt-2 w-64 bg-white border border-gray-200 rounded-sm shadow-xl z-50">
            ...
            {/* Subscription Management (hide only if EXCLUSIVELY Monday user) */}
...
function DashboardContent({ onSelectWheel, onShowProfile, currentView, setCurrentView, invitationCount, onInvitationAccepted, refreshInvitations }) {
  ...
  const handleCreateWheelClick = () => {
    // No limits for Monday users OR premium users
    if (mondayUserData?.isMondayUser || isPremium || isAdminUser) {
      setShowCreateModal(true);
      return;
    }
    
    // Check if free user has reached wheel limit
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex flex-col">
          ...
          <div className="flex justify-between items-center gap-4">
            ...
            <div className="flex items-center gap-3 sm:gap-8 min-w-0 flex-1">
              ...
              <nav className="hidden lg:flex items-center gap-1 sm:gap-2 overflow-x-auto">
                ...
                {/* Rapporter - Admin Only */}
                {isAdminUser && (
                  <button
                    data-cy="nav-templates"
                    onClick={() => navigate('/templates')}
                    className="flex items-center gap-1.5 sm:gap-2 px-2 sm:px-4 py-2 sm:py-2.5 rounded-sm font-medium transition-all whitespace-nowrap text-gray-600 hover:bg-gray-100 hover:text-gray-900"
                    title="Rapportmallar"
                  >
                    <svg className="w-4 h-4 sm:w-5 sm:h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                    </svg>
                    <span className="text-xs sm:text-sm">Rapporter</span>
                  </button>
                )}
              </nav>
            </div>
      ...
      {/* Upgrade Prompt Modal - Only show for free users (not Monday or Premium) */}
      {showUpgradePrompt && !mondayUserData?.isMondayUser && !isPremium && !isAdminUser && (
        <UpgradePrompt
